...
- `POST /api/v1/import` — CSV import for users and events (column-mapping preview)
...
`npx @anykpi/cli` (`packages/cli`): `login` (alias `key`), `workspaces`, `connect`, `import`, `identify`, `track`, `overview`, `users`, `cohorts`, `wbr`, `calendar`, `sync`. `login` requires an operator key. `connect` stores source config only. `import` loads a users or events CSV.
