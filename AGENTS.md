...
Shipped: PostHog, Mixpanel, Amplitude, Stripe, RevenueCat. Pull-only (Stripe also accepts a signature-verified webhook at `POST /api/webhooks/stripe`). Connector setup is the `/connect` UI or `anykpi connect`. Config is encrypted at rest with `ANYKPI_SECRET`. Env vars are a deprecated read-only fallback.
...
- `src/connectors` — PostHog, Mixpanel, Amplitude, Stripe, RevenueCat
