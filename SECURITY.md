...
- Ingest (`POST /api/ingest/event`, `POST /api/ingest/identify`, `POST /api/ingest/batch`), key minting (`POST /api/v1/keys`), MCP `tools/call` (except demo read-only tools), and all non-demo reads require a valid key.
...
- Humans can `POST /api/session` with a valid key to set a signed `httpOnly` + `SameSite=Lax` cookie for live dashboard reads. The cookie never contains the key. `DELETE /api/session` clears it. Ingest, connect, import, sync trigger, key mint/revoke, and person deletion (`DELETE /api/v1/users/{id}`) stay key-only so audit attribution does not become "someone with the secret". A deleted person is tombstoned so a later connector sync cannot resurrect them.
