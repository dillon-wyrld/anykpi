...
- Humans can `POST /api/session` with a valid key to set a signed `httpOnly` + `SameSite=Lax` cookie for live dashboard reads. The cookie never contains the key. `DELETE /api/session` clears it. Ingest, connect, import, sync trigger, and key mint/revoke stay key-only so audit attribution does not become "someone with the secret".
